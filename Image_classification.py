...
training_set = train_datagen.flow_from_directory('training_set',target_size = (224, 224),batch_size = batch_size,class_mode = cl_mode)
test_set = test_datagen.flow_from_directory('test_set',target_size = (224, 224),batch_size = batch_size,class_mode = cl_mode)
...
history = my_model.fit_generator(training_set,steps_per_epoch = round(len(train_samples)/batch_size,epochs = 25,
                       validation_data = test_set,validation_steps = round(len(test_samples)/batch_size ,callbacks=callbacks_list,shuffle=True,verbose=1)
